set G;               # generators
set T0;              # time periods including 0 (numeric)
...
# Commitment transition logic using numeric predecessor (t-1)
subject to Commitment_Logic {i in G, t in T}:
    u[i,t] - u[i,t-1] - v[i,t] + w[i,t] = 0;

# Minimum up time: if started within last tU periods, must be ON
subject to Min_Up_Time {i in G, t in T}:
    sum {k in T: k <= t and k >= t - tU[i] + 1} v[i,k] <= u[i,t];

# Minimum down time: if shut down within last tD periods, must be OFF
subject to Min_Down_Time {i in G, t in T}:
    sum {k in T: k <= t and k >= t - tD[i] + 1} w[i,k] <= 1 - u[i,t];
